...
//
                ...
                if (objective - pricingProblem.dualCostsMap.get("oneRoutePerWorkerAtMost")[pricingProblem.worker.getIndex()] <= -Constants.precisionForReducedCost) { //Generate new column if it has negative reduced cost
...
//                        System.out.println("column number:"+newColumns.size());
    ...
    protected void setObjective() {
        ...
        try {
            ...
            for (int i = 0; i < dataModel.instance.getCustomers().size(); i++) {
                ...
                c += pricingProblem.dualWC[pricingProblem.worker.getIndex()][i];
            ...
            for (int s = 0; s < dataModel.instance.getStationCandidates().size(); s++) {
                ...
                c = pricingProblem.dualWS[pricingProblem.worker.getIndex()][s];
                objExpNew.addTerm(-c, v[s]);
            }
    ...
    public void branchingDecisionPerformed(BranchingDecision bd) {
        try {
            if (bd instanceof RemoveLocation) {
                ...
                v[removeLocation.locationIndex].setUB(0);
                ...
                if (pricingProblem.worker.equals(fixWorkerStation.worker)) {
                    v[fixWorkerStation.stationCandidate.getIndex()].setLB(1);
                }
                ...
                if (pricingProblem.worker.equals(incompatibleWorkerStation.worker)) {
                    v[incompatibleWorkerStation.stationCandidate.getIndex()].setUB(0);
                }
                ...
                if (pricingProblem.worker.equals(fixWorkerCustomer.worker)) {
                    x[fixWorkerCustomer.customer.getIndex()].setLB(1);
                }
                ...
                if (pricingProblem.worker.equals(incompatibleWorkerCustomer.worker)) {
                    x[incompatibleWorkerCustomer.customer.getIndex()].setUB(0);
                }
            }
                ...
                IloConstraint branchingConstraint = cplex.addEq(x[sameColorDecision.vertexPair.getFirst().getIndex()], x[sameColorDecision.vertexPair.getSecond().getIndex()]);
                branchingPairConstraints.put(sameColorDecision.vertexPair, branchingConstraint);
                ...
                IloConstraint branchingConstraint = cplex.addLe(cplex.sum(x[differentColorDecision.vertexPair.getFirst().getIndex()], x[differentColorDecision.vertexPair.getSecond().getIndex()]), 1);
                branchingPairConstraints.put(differentColorDecision.vertexPair, branchingConstraint);
            }
    ...
    public void branchingDecisionReversed(BranchingDecision bd) {
        try {
            if (bd instanceof RemoveLocation) {
                RemoveLocation removeLocation = (RemoveLocation) bd;
                v[removeLocation.locationIndex].setUB(1);
            } else if (bd instanceof FixWorkerStation) {
                FixWorkerStation fixWorkerStation = (FixWorkerStation) bd;
                if (pricingProblem.worker.equals(fixWorkerStation.worker)) {
                    v[fixWorkerStation.stationCandidate.getIndex()].setLB(0);
                }
            } else if (bd instanceof IncompatibleWorkerStation) {
                IncompatibleWorkerStation incompatibleWorkerStation = (IncompatibleWorkerStation) bd;
                if (pricingProblem.worker.equals(incompatibleWorkerStation.worker)) {
                    v[incompatibleWorkerStation.stationCandidate.getIndex()].setUB(1);
                }
            } else if(bd instanceof FixWorkerCustomer){
                FixWorkerCustomer fixWorkerCustomer = (FixWorkerCustomer) bd;
                if (pricingProblem.worker.equals(fixWorkerCustomer.worker)) {
                    x[fixWorkerCustomer.customer.getIndex()].setLB(0);
                }
            }else if(bd instanceof IncompatibleWorkerCustomer){
                IncompatibleWorkerCustomer incompatibleWorkerCustomer = (IncompatibleWorkerCustomer) bd;
                if (pricingProblem.worker.equals(incompatibleWorkerCustomer.worker)) {
                    x[incompatibleWorkerCustomer.customer.getIndex()].setUB(1);
                }
            }
            else if (bd instanceof SameColumn) {
                SameColumn sameColorDecision = (SameColumn) bd;
                cplex.remove(branchingPairConstraints.get(sameColorDecision.vertexPair));
            } else if (bd instanceof DifferentColumn) {
                DifferentColumn differentColorDecision = (DifferentColumn) bd;
                cplex.remove(branchingPairConstraints.get(differentColorDecision.vertexPair));
            }
        } catch (IloException e) {
            e.printStackTrace();
        }
