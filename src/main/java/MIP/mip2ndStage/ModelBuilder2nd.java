...
public class ModelBuilder2nd {
    ...
    public ModelBuilder2nd(Instance dataModel, int TotalSceNum, List<Station> stations) throws IloException {
        this.dataModel = dataModel;
        this.TotalSceNum=TotalSceNum;
        this.stations=stations;
        this.buildModel();
    }
    public ModelBuilder2nd(Instance dataModel, List<Station> stations) throws IloException {
        new ModelBuilder2nd(dataModel,1,stations);
    }

    /**
     * Solve the root node of the Branch and Bound tree.
     */
    public MipData2nd getLP() throws IloException {
        mipData2nd.cplex.setParam(IloCplex.IntParam.NodeLim, 0);
        return mipData2nd;
    }
     ...
     */
    public MipData2nd getILP() throws IloException {
        mipData2nd.cplex.setParam(IloCplex.IntParam.NodeLim, 210000000); //Continue search
        //mipDataS.cplex.setParam(IloCplex.BooleanParam.PreInd, false);
        mipData2nd.cplex.setParam(IloCplex.DoubleParam.TiLim, 7200); //set time limit in seconds
        mipData2nd.cplex.setParam(IloCplex.IntParam.Threads, Constants.MAXTHREADS);
        mipData2nd.cplex.setParam(IloCplex.IntParam.NodeFileInd, 3);
...
//        //4. each customer is either visited by a worker or unServed
        ...
        for (int xi = 0; xi < scenarioNum; xi++) {
            for (int s = 0; s < stationNum; s++) {
                ...
                cplex.addLe(expr, stations.get(s).getCapacity(), "station capacity_s" + s + "_xi" + xi);
            }
        }
        //6. define z
        for (int xi = 0; xi < scenarioNum; xi++) {
...
//        }
        ...
        mipData2nd = new MipData2nd(cplex, varsAssignIK, tao, varsAssignSK, unServed, z, theta, dmax1, u, f1, varTheta, epsilon, dmax2, f2, a1plus, a1minus, b1plus, b1minus, a2plus, a2minus, b2plus, b2minus, gamma1, gamma2, gamma3, gamma4, gamma5, gamma6, gamma7, gamma8, gamma9,
