...
public class MipD {
    ...
    private ModelBuilderD modelBuilderD;
    ...
    int typeNum;
    public MipD( Instance dataModel) {

        new MipD(dataModel,1);

    }
    public MipD(Instance instance,int TotalSceNum) {
        this.dataModel = instance;
        stationNum = dataModel.getStationCandidates().size();
        ...
        varsLocationValues = new int[stationNum][typeNum];
        //second stage decision variable
        ...
        try {
            modelBuilderD = new ModelBuilderD(dataModel,TotalSceNum);
            mipDataD = modelBuilderD.getILP();
    ...
    public void solve() throws IloException {
        mipDataD.cplex.exportModel("mip.lp");
...
//		mipDataS.cplex.setParam(IntParam.Threads, 2);
        ...
        if (mipDataD.cplex.solve() && (mipDataD.cplex.getStatus() == IloCplex.Status.Feasible || mipDataD.cplex.getStatus() == IloCplex.Status.Optimal)) {
            ...
            double obj1st=0;
            double[] varsL=new double[dataModel.getStationCandidates().size()];
            for (int s = 0; s < dataModel.getStationCandidates().size(); s++) {
                for(int t=0;t<dataModel.getType().length;t++){
                    varsLocationValues[s][t] = MathProgrammingUtil.doubleToInt(mipDataD.cplex.getValue(mipDataD.varsLocation[s][t]));
            ...
            for (int xi = 0; xi < scenarioNum; xi++) {
                ...
                for (int k = 0; k < workerNum; k++) {
                    ...
                    for (int i = 0; i < customerNum; i++) {
                        varsAssignIKValues[i][k][xi] = mipDataD.cplex.getValue(mipDataD.varsAssignIK[i][k][xi]);
                ...
                for (int k = 0; k < workerNum; k++) {
                    ...
                    for (int s = 0; s < stationNum; s++) {
                        varsAssignSKValues[s][k][xi] = mipDataD.cplex.getValue(mipDataD.varsAssignSK[s][k][xi]);
                ...
                for (int i = 0; i < customerNum; i++) {
                    unServedValues[i][xi] = mipDataD.cplex.getValue(mipDataD.unServed[i][xi]);
...
//                }
                ...
                for (int k = 0; k < workerNum; k++) {
                    ...
                    dmax1Values[k][xi] = mipDataD.cplex.getValue(mipDataD.dmax1[k][xi]);
                    cost+=dataModel.getModelCoe()[3]*dmax1Values[k][xi];
                    for (int j = 0; j < workerCapacityNum+1; j++) {
                        uValues[k][j][xi] = mipDataD.cplex.getValue(mipDataD.u[k][j][xi]);
                    }

                    for (int j = 0; j < workerCapacityNum+1; j++) {
                        f1Values[k][j][xi] = mipDataD.cplex.getValue(mipDataD.f1[k][j][xi]);
                        cost+=dataModel.getModelCoe()[4]*f1Values[k][j][xi];
                    }

                    for (int s = 0; s < stationNum; s++) {
                        for (int i = 0; i < customerNum; i++) {
                            varThetaValues[i][s][k][xi] = mipDataD.cplex.getValue(mipDataD.varTheta[i][s][k][xi]);
                        }
                    }
...
//                }
                    ...
                    dmax2Values[k][xi] = mipDataD.cplex.getValue(mipDataD.dmax2[k][xi]);
                    cost+=dataModel.getModelCoe()[6]*dmax2Values[k][xi];


                    for (int j = 0; j < workerCapacityNum+1; j++) {
                        f2Values[k][j][xi] = mipDataD.cplex.getValue(mipDataD.f2[k][j][xi]);
                        cost+=dataModel.getModelCoe()[7]*f2Values[k][j][xi];

                    }

                    a1plusValues[k][xi] = mipDataD.cplex.getValue(mipDataD.a1plus[k][xi]);

                    a1minusValues[k][xi] = mipDataD.cplex.getValue(mipDataD.a1minus[k][xi]);

                    b1plusValues[k][xi] = mipDataD.cplex.getValue(mipDataD.b1plus[k][xi]);

                    b1minusValues[k][xi] = mipDataD.cplex.getValue(mipDataD.b1minus[k][xi]);
...
//			System.out.println("Solution:\n{}"+solution);
        } else if (mipDataD.cplex.getStatus() == IloCplex.Status.Infeasible) {
//			throw new RuntimeException("Mip infeasible");
            this.isFeasible = false;
            this.optimal = true;
        } else if (mipDataD.cplex.getCplexStatus() == IloCplex.CplexStatus.AbortTimeLim) {
            ...
            throw new RuntimeException("Cplex solve terminated with status: " + mipDataD.cplex.getStatus());
        }
        mipDataD.cplex.end();
    }
    ...
    public int getNrOfNodes() {
        return mipDataD.cplex.getNnodes();
    }
}
