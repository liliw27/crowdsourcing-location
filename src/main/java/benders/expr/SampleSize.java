...
//    List<Scenario>[] scenarioBatches = new List<>[M];
    ...
    public List<Scenario>[] getScenarioBatches(Instance instance, int N) {
        ...
        for (Customer customer : instance.getCustomers()) {
            ...
            gammacustomers[customer.getIndex()] = new GammaDistribution(shape, scale);
        }
        for (Worker worker : instance.getWorkers()) {
            double shape = 1 / (coeW * coeW);
            double scale = coeW * coeW * worker.getCapacity();
            gammaworkers[worker.getIndex()] = new GammaDistribution(shape, scale);
        }
        for (int m = 0; m < M; m++) {
...
//            cgSolver.solveCG();
    ...
    public static void main(String[] args) throws IOException, XGBoostError, IloException {
        ...
        Instance instance = Reader.readInstance(file, 50, 0, 10, 30, 30, 1);
        ...
        System.out.println("LNM: "+sampleSize.LNM+"; lb1: "+sampleSize.lb1+"; lb2: "+sampleSize.lb2);
        System.out.println("UB: "+sampleSize.UB+"; ub1: "+sampleSize.ub1+"; ub2: "+sampleSize.ub2);
